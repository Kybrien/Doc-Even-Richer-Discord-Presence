...
| Réglage | Défaut | Rôle |
|---|:---:|---|
| **Enabled** | ✅ | Interrupteur général. Décoché, le plugin ne fait plus rien du tout, même sur un `Connect` explicite. |
| **Client Id** | *vide* | Ton Application ID Discord. **Obligatoire.** Utilisé aussi par `Connect` quand sa pin est vide, et vérifié à la saisie. |
...
| **Only Connect On First Instance In PIE** | ✅ | Éditeur uniquement. Limite le Play In Editor à une seule connexion Discord au lieu d'une par client. |
...
**21 nœuds Blueprint, 6 events.** Chacun porte une infobulle dans l'éditeur.
...
<tr><td><b>Set Presence Links</b></td><td>Définit ou retire les deux liens cliquables. Fusionne dans la présence déjà posée.</td></tr>
...
Ce **n'est pas** « Set Presence avec plus d'options débloquées ». Exactement deux champs ne sont
...
*`Details Url` et `State Url` étaient ici aussi. Ils ont désormais leur propre nœud,
`Set Presence Links`, qui fusionne au lieu de remplacer.*
...
|---|:---:|---|
| **Connect** | | Se connecte. Laisse la pin `Client Id` vide pour reprendre celui des Project Settings, ou remplis-la pour le surcharger à l'exécution. Utile surtout pour choisir le moment. L'appeler sur une connexion déjà établie ne fait rien. |
...
des boutons. **Une party publiée est préservée** dans tous les cas : Discord refuse les boutons à côté
d'un join secret, mais pas à côté d'une party, donc passer des boutons coûte le Ask to Join et conserve
le compteur.
...
**Les liens survivent à un rafraîchissement de texte.** Une fois posés avec `Set Presence Links`,
`Details Url` et `State Url` restent sur la présence à travers tous les `Set Presence` suivants.
Rappelle le nœud avec les deux pins vides pour les retirer.
...
**Play In Editor :** chaque instance PIE a sa propre game instance et donc sa propre connexion vers le
même compte Discord, donc plusieurs clients écrasent mutuellement leur présence. Par défaut, seule la
**première** instance se connecte, les autres écrivent une ligne pour le dire. Décoche
**Only Connect On First Instance In PIE** pour toutes les laisser se connecter. Un `Connect` explicite
passe toujours outre la règle, donc une seconde instance reste connectable volontairement pour tester
le flux de join, et les builds packagées ignorent totalement ce réglage.

*Ce n'est pas le rate limit Discord qui est en cause : chaque instance a sa propre connexion et son
propre throttle, donc quatre clients font quatre connexions indépendantes, pas quatre appels empilés
sur une seule. Le problème est le bruit et le dernier qui écrit gagne.*

**Discord valide l'activité dans son ensemble.** Une seule URL de bouton malformée ou un seul champ
au-delà de sa limite de longueur, et plus rien n'arrive sur le profil, ni le texte ni l'art. Le plugin
vérifie ça avant d'envoyer et nomme le champ dans le log, en niveau `Error`, pour qu'un profil vide
cesse d'être un mystère.
...
| **Rien ne se passe et le log dit que le plugin est désactivé** | **Enabled** est décoché dans les Project Settings. `Connect` ne le contourne pas. |
| **Tout a disparu juste après l'ajout d'un bouton** | L'URL du bouton ne commence ni par `http://` ni par `https://`, ou un champ dépasse la limite de longueur Discord. Discord refuse l'activité entière. Le log nomme le champ. |
...
| Un seul client PIE affiche une présence | C'est voulu. **Only Connect On First Instance In PIE** est actif. Appelle `Connect` sur l'autre instance, ou décoche le réglage. |
| `LNK1104` sur `...suppressed.lib` au packaging | Limite de longueur de chemin Windows, pas le plugin. Rapproche le projet et le dossier de sortie de la racine du disque. |
...
| `Set Presence Links` ne fait rien | Aucune présence n'est publiée. Il fusionne dans la présence courante, donc appelle `Set Presence` d'abord. Un warning unique est émis. |
| Le hover text ne s'affiche jamais | La clé d'image correspondante est vide. Le texte est une infobulle sur l'image, sans image il n'y a rien à survoler. |
